/**
 * Single source of truth for the IES Essay Competition schedule.
 *
...
> = {
  upcoming: {
    ...
    body: `Registration for the IES Essay Competition opens ${formatDateTime(
      OPENS_AT,
  ...
  open: {
    ...
    body: `Entries close ${formatDateTime(
      DEADLINE_AT,
    )}. The prompt is emailed to every registrant when the competition window begins.`,
  },
  competition: {
    label: "In progress",
    ...
    body: `Entries closed ${formatDateTime(
      DEADLINE_AT,
    )}. The competition runs through ${formatDate(COMPETITION_ENDS_AT)}.`,
  },
  closed: {
    label: "Closed",
